...
class NotificationsBloc extends Bloc<NotificationsEvent, NotificationsState> {
  ...
  NotificationsBloc({
    ...
    required RemoveScheduledNotificationUseCase removeScheduledNotificationsUseCase,
    required SaveScheduledNotificationUseCase saveScheduledNotificationUseCase,
       ...
       _removeScheduledNotificationsUseCase = removeScheduledNotificationsUseCase,
      ...
      for (int i = 0; i < event.words.length; i++) {
        ...
        await _saveScheduledNotificationUseCase.execute(
          ScheduledNotificationEntity(
            ...
            title: word.word,
            body:
                word.senses.firstOrNull?.definition ?? 'Learn the meaning of this word!',
  ...
  _reminderWordTomorrow(
    ...
    if (isGranted) {
      ...
      final isScheduled = pendingNotifications.any(
        (element) => element.id == event.word.index,
      );
      if (isScheduled) {
        debugPrint('NotificationsBloc: reminderWordTomorrow isScheduled: $isScheduled');
        emit(
          state.copyWith(
            failure: Failure.general(message: 'Notification is already scheduled'),
          ),
        );
