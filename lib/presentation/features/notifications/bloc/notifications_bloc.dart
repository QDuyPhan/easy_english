...
class NotificationsBloc extends Bloc<NotificationsEvent, NotificationsState> {
  ...
  final RemoveScheduledNotificationUseCase _removeScheduledNotificationsUseCase;
  final SaveScheduledNotificationUseCase _saveScheduledNotificationUseCase;
  ...
  NotificationsBloc({
    ...
    required RemoveScheduledNotificationUseCase
    removeScheduledNotificationsUseCase,
    required SaveScheduledNotificationUseCase saveScheduledNotificationUseCase,
  }) : _localNotificationsTools = localNotificationsTools,
       _getScheduledNotificationsUseCase = getScheduledNotificationsUseCase,
       _removeScheduledNotificationsUseCase =
           removeScheduledNotificationsUseCase,
       _saveScheduledNotificationUseCase = saveScheduledNotificationUseCase,
    ...
    on<NotificationsEvent>((event, emit) async {
      await event.map(
        ...
        handleOpenAppFromNotification:
            (event) => _onHandleOpenAppFromNotification(event, emit),
        clearWordIdFromNotification:
            (event) => _onClearWordIdFromNotification(event, emit),
        scheduleNextDayReminder:
            (event) => _scheduleNextDayReminder(event, emit),
        ...
        getScheduledNotifications:
            (event) => _onGetScheduledNotifications(event, emit),
        removeScheduledNotifications:
            (event) => _onRemoveScheduledNotifications(event, emit),
  ...
  _requestPermissions(
    ...
    if (oldGrantedState == false && isGranted == true) {
      ...
      add(
        NotificationsEvent.scheduleNextDayReminder(
  ...
  _scheduleNextDayReminder(
    ...
    if (isGranted) {
      ...
      await _saveScheduledNotificationUseCase.execute(
  ...
  _reminderWordTomorrow(
    ...
    if (isGranted) {
      ...
      final isScheduled = pendingNotifications.any(
        (element) => element.id == event.word.id,
      );
      if (isScheduled) {
        debugPrint(
        ...
        emit(
          state.copyWith(
            failure: Failure.general(
              message: 'Notification is already scheduled',
            ),
          ),
        );
  ...
  _onHandleOpenAppFromNotification(
    _HandleOpenAppFromNotification event,
    Emitter<NotificationsState> emit,
  ) async {
  ...
  _onClearWordIdFromNotification(
    _ClearWordIdFromNotification event,
    Emitter<NotificationsState> emit,
  ) {
